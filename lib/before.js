...
const { beforeMiddleware } = common;
const { getController, createTree } = common;
...
module.exports = function router({ root: app }) {
   ...
   app.router = {
      get(path, ...middlewares) {
         ...
         createTree(app, 'GET', path, middlewares, controller);
      },
      post(path, ...middlewares) {
         const controller = getController(app, path, middlewares);
         createTree(app, 'POST', path, middlewares, controller);
      },
      put(path, ...middlewares) {
         const controller = getController(app, path, middlewares);
         createTree(app, 'PUT', path, middlewares, controller);
      },
      delete(path, ...middlewares) {
         const controller = getController(app, path, middlewares);
         createTree(app, 'DELETE', path, middlewares, controller);
      },
      /**
       * Rest api
       * @param {String} path 
         ...
         if (typeof controller === 'object') {
            ...
            for (const name in Resources) {
               ...
               if (action instanceof Function) {
                  ...
                  createTree(app, type, path + params, middlewares, action);
               }
            }
